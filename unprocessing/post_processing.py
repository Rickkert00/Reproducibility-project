...
from unprocessing.unprocess_images import metadata

import tensorflow as tf


def apply_gains(image, red_gains, blue_gains):
...
def apply_ccms(images, ccm):
  """Applies color correction matrices."""
  # images.shape.assert_has_rank(4)
  images = images[:, :, tf.newaxis, :]
  ccm = ccm[tf.newaxis, tf.newaxis, :, :]
  return tf.reduce_sum(images * ccm, axis=-1)


def gamma_compression(image, gamma=2.2):
  """Converts from linear to gamma space."""
  # Clamps to prevent numerical instability of gradients near zero.
  return tf.maximum(image, 1e-8) ** (1.0 / gamma)
...
def process(image):
  ...
  red_gain, blue_gain = metadata["red_gain"], metadata["blue_gain"]
  with tf.name_scope('process'):
    ...
    # Color correction.
    image = apply_ccms(image, cam2rgb)  # use the cam2rgb stored in the json
    # Gamma compression.
...
if __name__ == '__main__':
  img = tf.io.read_file("../data/nerf_raw_noise/lego/train/r_0.png")
  raw_file = tf.io.decode_png(img, channels=3)
  white_level = 255.0
  raw_image = tf.cast(raw_file, tf.float32) / white_level
  png_image = process(raw_image)
  png_img_white_level = png_image * 255.0
  png_numpy_img = png_img_white_level.numpy()
  tf.keras.utils.save_img("test_processed.png", png_numpy_img, scale=False)
